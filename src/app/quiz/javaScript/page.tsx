...
const QuizPage = () => {
  ...
  const [isFirstQuestionSubmitted, setIsFirstQuestionSubmitted] = useState<boolean>(false); // New state to track if the first question has been submitted
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        setQuizTitle(data.quizzes[2].title);
      } catch (error) {
  ...
  const handleSubmit = () => {
    if (selectedAnswers[currentQuestion]) {
      ...
      setTimeout(() => {
        ...
        if (currentQuestion === 0 && !isFirstQuestionSubmitted) {
          // Set the flag if the first question is being submitted
    ...
    >
      ...
      >
        ...
        <div className="mb-8">
          ...
          {isFirstQuestionSubmitted && ( // Conditionally render the progress bar
